...
export default function Page(){
  // Hook de categorías (se mantiene igual)
  ...
  useEffect(() => {
  // Solo establecer categoría por defecto si:
  // 1. Las categorías están cargadas
  // 2. No hay categoría activa
  // 3. No hay término de búsqueda (para no interferir con búsquedas)
  if (
    categories?.getAllCategories && 
    categories.getAllCategories.length > 0 && 
    !activeCategory &&
    !searchTerm.trim() // <- Agregar esta condición
  ) {
    const firstCategory = categories.getAllCategories[0];
    setActiveFilter(firstCategory.name, firstCategory.id);
  }
}, [categories, activeCategory, searchTerm, setActiveFilter]);

  useEffect(() => {
    if (products) console.log('Products fetched:', products.findProductsByRelation);
      ...
      <div className="container mx-auto px-4 py-8 mb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            {!loadingCategories && categories?.getAllCategories && (
              <div className="flex flex-wrap gap-2 mb-8">
                {categories.getAllCategories.map((category) => (
                  <button
                    key={category.id}
                    className={`px-4 py-2 rounded-full text-sm transition-all duration-300 ease-in-out transform hover:scale-105 ${activeCategory === category.name
            ...
            {!loadingProducts && products?.findProductsByRelation?.products && (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {products.findProductsByRelation.products.map((product, index) => (
                  <div 
                    key={product.id} 
                    className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 ease-in-out transform hover:scale-105 animate-fade-in-up"
                    style={{
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <button className="w-full bg-purple-600 hover:bg-purple-700 text-white py-2 px-4 rounded-md flex items-center justify-center gap-2 transition-all duration-200 hover:shadow-md">
            ...
            {!loadingProducts && products?.findProductsByRelation?.products?.length === 0 && (
